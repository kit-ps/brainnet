...
X=np.load('./Data/D_1_X.npy')*1000000
Y=np.load('./Data/D_1_Y.npy')

#X=np.load('./Data/D_2_X.npy')*1000000
#Y=np.load('./Data/D_2_Y.npy')
...
def assessment_model_data_closed():
	from sklearn.metrics.pairwise import cosine_similarity as cs
	...
	for i in range(len(x_test_1)):
		...
		#prediction=cs(anc_e,tm2)
		#pair1.append(test_e)
		#pair2.append(anc_e) a
		#cosine_similarity = metrics.CosineSimilarity()
		#print(calsstrain, y_train_val[i],calsstrain == y_train_val[i],len(digit_indices))
	...
	#resutls = np.array(resutls)
		...
		for j in calsstrain:
			#print(calsstrain,j,np.where(calsstrain == j)[0][0],len(digit_indices))
...
#for train_index, test_index in skf.split(X, Y, groups=Y):
  ...
  #for i in range(30):
  train_dataset = tf.data.Dataset.from_tensor_slices((x_train_val, y_train_val)).shuffle(1000).batch(256)
...
import pickle
with open('./Similarity Scores/d1_dicr1.pkl', 'wb') as f:
    pickle.dump(dicr1, f)

with open('./Similarity Scores/d1_dicr2.pkl', 'wb') as f:
    pickle.dump(dicr2, f)

with open('./Similarity Scores/d1_dicr3.pkl', 'wb') as f:
    pickle.dump(dicr3, f)
